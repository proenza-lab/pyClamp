    ...
    if LOOP:  # save some CPU instead of querying the widget
        try:
            ...
            if not count % REDRAW:  # redraw in intervals
                # compute new data limits
                PYCLAMP.figure.gca().relim(visible_only=True)
                # autoscale view based on new data limits
                PYCLAMP.figure.gca().autoscale_view(scalex=False, scaley=True)
                # update axes despite active blitting
                PYCLAMP.canvas.draw()
    return LINES
